// Shared Unicode style transforms used by the font generator tools.
...
export const fontbStyles = styleLines.split('\n').map(line => {
...
export const STYLE_FINGERPRINT_PROBE = 'abcdefghijklmnopqrstuvwxyz ABCDEFGHIJKLMNOPQRSTUVWXYZ 0123456789';

const unique = (values) => [...new Set(values.filter(Boolean))];

function createStyleRecord(style) {
  return {
    ...style,
    aliases: [],
    aliasNames: [],
    aliasIds: [],
    searchableTags: unique(style.tags),
    transform: (text) => transformStyle(style, text)
  };
}

function addAlias(canonical, style) {
  const alias = {
    id: style.id,
    name: style.name,
  ...
  canonical.aliasNames = canonical.aliases.map(item => item.name);
  canonical.aliasIds = canonical.aliases.map(item => item.id);
  canonical.searchableTags = unique([
    ...canonical.tags,
    ...canonical.aliases.flatMap(item => item.tags),
    ...canonical.aliases.map(item => item.name),
    ...canonical.aliases.map(item => item.id)
  ]);
  return alias;
}

function buildStyleRegistry(definitions) {
  const byId = new Map();
  const outputToStyle = new Map();
  const styles = [];
  const aliases = [];
  const aliasGroups = [];
  ...
  for (const definition of definitions) {
    if (byId.has(definition.id)) {
      throw new Error(`Duplicate font style id: ${definition.id}`);
    }
    byId.set(definition.id, definition);
    const fingerprint = transformStyle(definition, STYLE_FINGERPRINT_PROBE);
    const canonical = outputToStyle.get(fingerprint);
    if (canonical) {
      aliases.push(addAlias(canonical, definition));
    } else {
      const record = createStyleRecord(definition);
      outputToStyle.set(fingerprint, record);
      styles.push(record);
    }
  }

  for (const style of styles) {
    if (style.aliases.length) {
      aliasGroups.push({
  ...
  const canonicalById = new Map(styles.map(style => [style.id, style]));
  const aliasToCanonical = new Map(aliases.map(alias => [alias.id, canonicalById.get(alias.canonicalId)]));
...
export const styleRegistry = buildStyleRegistry(fontbStyles);
export const styles = styleRegistry.styles;
export const styleAliases = styleRegistry.aliases;
export const styleAliasGroups = styleRegistry.aliasGroups;
export const resolveStyle = (id) => styleRegistry.byId.get(id) || styleRegistry.aliasToCanonical.get(id) || null;
